...
class Interaction:
        ...
        for obj in sorted(self.sprites.list_of_objects, key=lambda obj: obj.distance_to_sprite):
            if self.player.shot and self.drawing.shot_animation_trigger:
                if obj.is_on_fire[1]:
                    ...
                    # opening door condition (weapon's shot to the door)
                    if obj.flag in {'door_h', 'door_v'}:
                        obj.door_open_trigger = True
                        obj.blocked = None

                    # exit loop to prevent bullet kill more than one object/npc
                    break

            # one more opening door condition (when distance < 1 TILE)
    ...
    def npc_move(self, obj):
        """
        NPC moves to the player, when distance to him exceeds 1 TILE
        :param obj:
        :return:
        """
    ...
    def check_win(self):
        """ check for Game over, when no npc left """
